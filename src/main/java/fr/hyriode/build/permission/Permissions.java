...
public class Permissions {
    ...
    public enum Action {
        ...
        UPLOAD((rank, environment) -> rank.isSuperior(StaffRank.STAFF)),
        RE_UPLOAD((rank, environment) -> {
            if (environment == Environment.PRODUCTION) {
                return rank.isSuperior(StaffRank.MANAGER);
            } else if (environment == Environment.DEVELOPMENT) {
                return rank.isSuperior(StaffRank.STAFF);
            }
            return false;
        }),
        IMPORT((rank, environment) -> rank.isSuperior(StaffRank.STAFF)),
        TRANSFER((rank, environment) -> rank.isSuperior(StaffRank.MANAGER) || rank.is(StaffRank.DEVELOPER)),
